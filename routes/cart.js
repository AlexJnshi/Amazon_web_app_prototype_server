...
router.get("/",(req,res)=>{
    pool.query(`select * from cart,product,product_img where cart.user_id=? and cart.product_id=product.pid and product_img.pidd=product.pid`,[req.query.uid],(err,result)=>{
        if(err){
            console.log(err)
            res.send({code:0});
        }else{
...
router.get("/update",(req,res)=>{
    if(req.query.count>0){
        pool.query(`update cart set count=? where user_id=? and product_id=?`,[req.query.count,req.query.uid,req.query.pid],(err,result)=>{
            if(err){
                console.log(err)
                res.send({code:0});
            }else{
        ...
        pool.query(`delete from cart where user_id=? and product_id=?`,[req.query.uid,req.query.pid],(err,result)=>{
            if(err){
                console.log(err)
                res.send({code:0});
            }else{
